...
/*
 ...
 PhoneConnectionManager.shared.onUIStateChanged = { [weak self] state in
 self?.updateConnectionUI(state: state)
 }
 
 PhoneConnectionManager.shared.onDataReceived = { [weak self] data in
 ...
 PhoneConnectionManager.shared.sendData(values, guaranteed: false)
 }
 }
 
 Usage on Apple Watch:
 
 class InterfaceController: WKInterfaceController {
 override func awake(withContext context: Any?) {
 super.awake(withContext: context)
 
 WatchConnectionManager.shared.onUIStateChanged = { [weak self] state in
 self?.updateConnectionUI(state: state)
 }
 
 WatchConnectionManager.shared.onDataReceived = { [weak self] values in
 self?.updateDataDisplay(values: values)
 }
 }
